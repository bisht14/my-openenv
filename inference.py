...
def get_model_action(client, obs_dict, step, last_feedback):
    ...
    if not API_KEY:
        return CodeReviewAction(
            issues_found=["Basic code issue detected"],
            severity_tags=["low"],
            explanation="Fallback response due to missing API key",
        )
    ...
    try:
        ...
        print(f"[SAFE FALLBACK] {e}", flush=True)

        return CodeReviewAction(
            issues_found=[
                "Potential bug in logic",
                "Missing validation"
            ],
            severity_tags=["medium", "low"],
            explanation="Fallback response ensuring stability",
        )
